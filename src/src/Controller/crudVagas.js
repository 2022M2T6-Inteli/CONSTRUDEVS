...
export async function selectVagas(req, res) {
  bancoDados().then((db) => {
    db.all("SELECT * FROM Vagas").then((Vagas) => res.json(Vagas));
  });
}
...
export async function selectVaga(req, res) {
  ...
  bancoDados().then((db) => {
    db.get("SELECT * FROM Vagas WHERE id_vagas=?", [id_vagas]).then((Vaga) =>
      res.json(Vaga)
    );
  });
}
...
export async function createVagas() {
  bancoDados().then((db) => {
    db.exec(
      "CREATE TABLE IF NOT EXISTS Vagas (id_vagas INTEGER PRIMARY KEY, regiao TEXT, area_de_atuacao TEXT, telefone INTEGER)"
    );
  });
}
...
export async function insertVaga(req, res) {
  ...
  bancoDados().then((db) => {
    db.run(
      "INSERT INTO Vagas (regiao, area_de_atuacao, telefone) VALUES (?,?,?)",
      [vagas.regiao, vagas.area_de_atuacao, vagas.telefone]
    );
  });
  res.json({
    statusCode: 200,
...
export async function updateVaga(req, res) {
  ...
  bancoDados().then((db) => {
    db.run(
      "UPDATE Vagas SET regiao=?, area_de_atuacao=?, telefone=? WHERE id_vagas=?",
      [vagas.regiao, vagas.area_de_atuacao, vagas.telefone, vagas.id_vagas]
    );
  });
  res.json({
    statusCode: 200,
...
export async function deleteVaga(req, res) {
  let id_vagas = req.body.id_vagas;
  bancoDados().then((db) => {
    db.get("DELETE FROM Vagas WHERE id_vagas=?", [id_vagas]).then((res) => res);
  });
  res.json({
    statusCode: 200,
  });
...
export async function joinEmpreiteiroVaga(req, res) {
  let atuacaoVagas = req.body.area_de_atuacao;
  let especialidadeEmpreiteiro = req.body.especialidade;
  bancoDados().then((db) => {
    db.all(
        "SELECT * FROM empreiteiros JOIN Vagas ON empreiteiros.especialidade = Vagas.area_de_atuacao",
      [especialidadeEmpreiteiro, atuacaoVagas]
